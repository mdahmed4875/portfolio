import React from 'react';
import "./Projects.css";

function Projects() {
  return (
    <section className="projects">
      <h2>Projects</h2>

      <div className="project-card">
        <h3>Portfolio Website</h3>
        <p>A responsive personal portfolio built using React and CSS.</p>
        <div className="project-links">
          <a href="https://github.com/yourusername/portfolio" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>

      <div className="project-card">
        <h3>Blogify</h3>
        <p>A backend blog platform using Node.js, Express, ejs and MongoDB.</p>
        <div className="project-links">
          <a href="https://github.com/mdahmed4875/blogify.git" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>
      <div className="project-card">
        <h3>URL shortner</h3>
        <p>A URL shortener built using Node.js, Express, MongoDB, and EJS that allows users to generate and manage short links efficiently</p>
        <div className="project-links">
          <a href="https://github.com/mdahmed4875/url-shortner.git" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>

      <div className="project-card">
        <h3>Meme Generator</h3>
        <p>A meme generator app using React and css.</p>
        <div className="project-links">
          <a href="https://github.com/mdahmed4875/meme-generator.git" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>
      <div className="project-card">
        <h3>Pager App</h3>
        <p>A Pager application using React and css.</p>
        <div className="project-links">
          <a href="https://github.com/mdahmed4875/pager.git" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>
       <div className="project-card">
        <h3>Time-Zone</h3>
        <p>A web app built with HTML, CSS, and JavaScript that displays the current time and location, with the ability to search and view the time in other cities worldwide</p>
        <div className="project-links">
          <a href="https://github.com/mdahmed4875/TimeZone.git" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>
      <div className="project-card">
        <h3>Todo App</h3>
        <p>A Todo app built with HTML, CSS, and JavaScript</p>
        <div className="project-links">
          <a href="https://github.com/mdahmed4875/Todo-App.git" target="_blank" rel="noopener noreferrer">GitHub</a>
        </div>
      </div>
      
     
    </section>
  );
}

export default Projects;
